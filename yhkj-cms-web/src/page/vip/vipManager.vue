      ...
      <div id="main">
        ...
        <div class="content-box-ipt box-border-b">
          ...
          <div class="mr-top head-input">
            <span>输入搜索：</span><el-input placeholder="手机号码 / 姓名" class="input-width" v-model="searchInfo.keyword" @keyup.enter.native="getUserList(true)"></el-input><span>所属门店：</span><el-select v-model="searchInfo.store" placeholder="请选择">
            <el-option
        ...
        <div class="box-content clearfix">
          <div class="mr-bottom clearfix">
            ...
            <el-row class="fll" v-if="importStatus">
              <el-button type="primary" @click="importUserVisible = true" style="margin-left: 1rem">调入</el-button>
            </el-row>
          </div>
          ...
          <el-dialog title="导入档案" :visible.sync="dialogArchives">
            ...
            <div slot="footer" class="dialog-footer">
              <el-button type="primary" @click="submitUpload" v-loading.fullscreen.lock="uploadStatus">确认上传</el-button>
            </div>
          </el-dialog>
        ...
        <el-dialog title="编辑会员" :visible.sync="editUserVisible">
          <el-form :rules="rules">
            ...
            <el-form-item label="性别" :label-width="formLabelWidth">
              <el-radio v-model="editUserInfo.sex" label='1' value='男性'>男性</el-radio>
              <el-radio v-model="editUserInfo.sex" label='2' value='女性'>女性</el-radio>
            </el-form-item>
            <el-form-item label="出生日期" :label-width="formLabelWidth">
              <el-date-picker
                format="yyyy 年 MM 月 dd 日"
        ...
        <el-dialog title="调入会员" :visible.sync="importUserVisible">
          <el-form>
            <el-form-item label="手机号">
              <el-input v-model="importSearchPhone" style="width: 15rem"></el-input>
              <el-button @click="importSearchClick">搜索</el-button>
            </el-form-item>
            <el-form-item label="用户名">
              <el-input v-model="importUserData.name" disabled="true" style="width: 15rem"></el-input>
            </el-form-item>
          </el-form>
          <div class="dialog-footer">
            <el-button @click="importUserVisible = false">取 消</el-button>
            <el-button type="primary" @click="importClick">调 入</el-button>
          </div>
        </el-dialog>
      </div>
    </section>
...
<script>
  ...
  import {searchPhone, vipList} from '../../service/getData'
  ...
  import {searchVip} from "../../service/getData";
  ...
  export default {
    ...
    methods:{
      ...
      async importClick(){
        let info  = await moveVip(this.importUserData.id);
        if(info.code === 0){
          this.$message.success('调入会员成功！');
          this.importUserVisible = false;
        }else {
          this.$message.error(info.msg);
        }
      },
      async importSearchClick(){
        if(this.importSearchPhone === ''){
          this.$message.error('请输入手机号');
          return 0;
        }
        let info = await searchPhone(this.importSearchPhone);
        this.importUserData = info.data.detail;
        return 0;
      },
      async tableOptions(id,keyword){
        switch (keyword){
          ...
          case 2:{
            ...
            this.editUserInfo.sex = this.editUserInfo.sex.toString();
    ...
    mounted(){
      let info = JSON.parse(window.localStorage.permission);
      this.getUserList();
      this.getMendianList();
    }
  }

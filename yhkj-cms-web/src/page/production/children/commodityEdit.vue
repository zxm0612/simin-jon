      ...
      <div id="main">
        ...
        <div class="content-box-input">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="商品类型：" prop="resource">
              <el-radio-group v-model="ruleForm.shoppingType">
                <el-radio v-model="ruleForm.shoppingType" :label="1" class="mr-right" @change="radioClick(1,1)" value=1 :disabled="!timesStatus">次卡</el-radio>
                <el-select v-model="ruleForm.shoppingCategory" :placeholder="timesPlace" class="mr-right" :disabled="!timesStatus" v-if="timesStatus">
                  <el-option v-for="item in timesInfo" :label="item.name" :value="item.id" :key="item.id"></el-option>
                </el-select>
                <el-radio v-model="ruleForm.shoppingType" :label=2 class="mr-right" @change="radioClick(2,1)" value=2 :disabled="!lcStatus">疗程卡</el-radio>
                <el-select v-model="ruleForm.shoppingCategory" :placeholder="lcPlace" :disabled="!lcStatus" v-if="lcStatus">
                  <el-option v-for="item in lcInfo" :label="item.name" :value="item.id" :key="item.id"></el-option>
                </el-select>
                <el-radio v-model="ruleForm.shoppingType" :label="3" class="mr-left" @change="radioClick(3,1)" value=3 :disabled="disHealthStatus">健康管理卡</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="商品价格：" prop="">
              <el-col style="width:4.4rem">一档价格：</el-col><el-col style="width:15rem"><el-input v-model="ruleForm.firstPrice " @blur="checkPrice"></el-input></el-col>
              <el-col style="width:4.4rem">二档价格：</el-col><el-col style="width:15rem"><el-input v-model="ruleForm.secondPrice" @blur="checkPrice"></el-input></el-col>
              <el-col style="width:4.4rem">三档价格：</el-col><el-col style="width:15rem"><el-input v-model="ruleForm.thirdPrice" @blur="checkPrice"></el-input></el-col>
            </el-form-item>
            ...
            <el-form-item label="包括项目：" prop="">
              ...
              <el-table :data="ruleForm.productList" border class="mr-top" v-if="lcStatus">
                ...
                <el-table-column prop="num" label="数量">
                  <template slot-scope="scope">
                    <p><el-input :value="scope.row.num" v-model="scope.row.num" @change="totalCompute" @blur="checkNum(scope.row.num)"></el-input></p>
                  </template>
                </el-table-column>
          ...
          <el-dialog title="添加" :visible.sync="secondaryCardVisible" z-index="10000">
            <el-input v-model="dialogCourseCardSearchInfo" placeholder="次卡名称" @keyup.enter.native="dialogSearch(1)"></el-input>
            ...
            <div class="paging-postion" style="">
              <el-pagination
          ...
          <!--次卡对话框结束-->
          <!--疗程卡对话框开始-->
          <el-dialog title="添加" :visible.sync="courseCardVisible" z-index="100000" @close="courseCardVisible = false">
            <el-input v-model="dialogCourseCardSearchInfo" placeholder="疗程卡名称" @keyup.enter.native="dialogSearch(2)"></el-input>
          ...
          <el-dialog title="添加" :visible.sync="healthyCardVisible" z-index="100000">
            <el-input v-model="dialogCourseCardSearchInfo" placeholder="疗程卡名称" @keyup.enter.native="dialogSearch(3)"></el-input>
            <el-table :data="dialogData" border class="mr-top" ref="selectTimes"  @selection-change="handleSelectionChange">
              <el-table-column width="99">
                <template slot-scope="scope">
                  <el-checkbox label="选择"  v-model="selectList[scope.$index]" :value="scope.row.id" @change="addProduct(scope.$index,selectList[scope.$index])"></el-checkbox>
  ...
  export default {
    data () {
      ...
      return {
        ...
        selectTimes:'',//选择的次卡项目
        selectList:[false,false,false,false,false,false,false,false,false,false],
        content:'',
        productStatus:[],//项目临时状态
        dialogCourseCardSearchInfo:''
      }
    },
    methods: {
      checkNum(num){
      ...
      checkPrice(){
        if(parseFloat(this.ruleForm.firstPrice).toString() === 'NaN'){
          this.ruleForm.firstPrice ='';
          this.$message.error('请输入数字！');
        }else if(parseFloat(this.ruleForm.secondPrice).toString() === 'NaN'){
          this.ruleForm.secondPrice ='';
          this.$message.error('请输入数字！');
        }else if(parseFloat(this.ruleForm.thirdPrice).toString()==='NaN'){
      ...
      async getProjectList(name ='',status=false){
        ...
        this.dialogLinshi = this.dialogData;
      ...
      addClick(){
        switch(this.ruleForm.shoppingType){
          ...
          case 2:{
            for(let i =0;i<this.ruleForm.productList.length;i++){
              delete this.ruleForm.productList[i].num;
      ...
      dialogSearch(key){
        switch (key) {
          case 1:{
            // this.dialogData = [];
            // for(let i = 0;i<this.dialogLinshi.length;i++){
            //   if(this.dialogLinshi[i].name.indexOf(this.dialogCourseCardSearchInfo) !== -1){
            //     this.dialogData.push(this.dialogLinshi[i]);
            //   }
            // }
            this.getProjectList(this.dialogCourseCardSearchInfo);
            // this.selectList = [false,false,false,false,false,false,false,false,false,false];
            break;
          }
          case 2:{
            //说明是疗程卡
    ...
    async mounted(){
      ...
      editor.create();
      await this.getLis0tData();
      await this.getMendianData();
